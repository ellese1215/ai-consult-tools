...
```powershell
pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\your-repo" -CaseName "base_maint" -IncludePaths "ai-consult-tools/claude/00_ai_consult_operation_rules.md","ai-consult-tools/claude/make_consult_bundle.ps1"
```

### 特定フォルダ + 運用ルール

```powershell
pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\your-repo" -CaseName "auth_feature" -IncludePaths "ai-consult-tools/claude/00_ai_consult_operation_rules.md","src/auth"
```

### 複数ファイル指定

```powershell
pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\your-repo" -CaseName "config_check" -IncludePaths "src/Config.php","src/Router.php","ai-consult-tools/claude/00_ai_consult_operation_rules.md"
```
...
```powershell
# 未コミット差分（適用直後）
pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode diff -RepoRoot "C:\your-repo" -CaseName "<相談名>"

# staged差分（git add後）
pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode diff -RepoRoot "C:\your-repo" -CaseName "<相談名>" -Staged
```
...
| 問題 | 対処 |
|---|---|
| `powershell.exe`で実行してしまった | `pwsh`（PowerShell 7+）で再実行する |
| 同名ファイルが複数ヒットしてエラーになった | `-IncludePaths` にフルパス（相対パス）で指定する |
| diff modeで差分が0件だった | 変更が未保存か、すでにコミット済みの可能性がある |
| 出力が_part1.md / _part2.mdに分割された | 両方のファイルをClaudeに添付する |
