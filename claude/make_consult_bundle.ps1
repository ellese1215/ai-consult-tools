...
Usage examples:
  ...
  # Mode D: map（本文なし軽量地図）
  # ------------------------------------------------------------
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode map -RepoRoot "C:\xampp\htdocs"

  # ------------------------------------------------------------
  # Mode C: repo（全体横断スナップショット）
  # ------------------------------------------------------------
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode repo -RepoRoot "C:\xampp\htdocs"

  # 設定ファイルを明示する例（未指定時は ai-consult-tools\claude\consult.config.json → .consult\consult.config.json の順に自動探索）
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode repo -RepoRoot "C:\xampp\htdocs" -ConfigPath ".consult\consult.config.json"

  # 複数行（読みやすさ重視）
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 `
    -Mode repo `
  ...
  # ------------------------------------------------------------
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\xampp\htdocs" -IncludePaths "common"

  # 複数パス指定（配列）
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\xampp\htdocs" -IncludePaths "common","admin","db\schema"

  # v1.4.5: ファイル名のみ/フォルダ名のみ指定（同名複数ヒット時は停止 / ワイルドカード非対応）
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\xampp\htdocs" -IncludePaths "Navigation.php","Loader.php"
  ...
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode diff -RepoRoot "C:\xampp\htdocs"

  # staged 差分
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode diff -RepoRoot "C:\xampp\htdocs" -Staged

  # ref 間差分
  pwsh -File ai-consult-tools\claude\make_consult_bundle.ps1 -Mode diff -RepoRoot "C:\xampp\htdocs" -DiffBase HEAD~1 -DiffTarget HEAD
#>
...
function Assert-RequiredPwsh {
    ...
    if ($null -eq $currentVersion -or $currentVersion.Major -lt 7) {
        ...
        $scriptPath = if ($PSCommandPath) { $PSCommandPath } else { "ai-consult-tools\claude\make_consult_bundle.ps1" }
