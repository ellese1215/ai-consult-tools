...
旧ツール配布ZIP、過去bundle、通常ログ、失敗ログ、重複資料は次の工程へ渡さない。複数ZIPを入れ子にしたhandoff compositeは、明示的な必要がある場合だけ作る。
...
- review bundle：実装後のbaseからtargetまでの差分
...
実在する変更対象だけを指定する。review時にstart bundleを作り直さない。
...
- レビューは正確なbaseからtargetまでを対象にし、仕様適合、変更漏れ、副作用、試験を確認する。
