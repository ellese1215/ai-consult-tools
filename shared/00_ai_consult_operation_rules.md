...
> Updated: 2026-06-26
> Version: 3.1.5
> Note: v3.1.5 ではDocSetを手動管理対象ではなく参照束IDとして整理し、ChatGPT版の include set / PATH_INDEX 運用を追加。
...
- includeするファイルのパスが不明な場合、パスを推測してinclude生成コマンドを提示しない。既存の include set、bundle内の PATH_INDEX、または map モードで所在を明らかにしてから include を要求する
...
AIは作業開始前に「唯一の正（参照束）」を明言し、BundleLabel / 参照束ID（DocSet）を引用して一致確認を完了するまで仕様案・修正案・コード案を提示しない。ここでいうDocSetは生成物IDであり、通常ドキュメントを手動更新するための管理番号ではない。
...
**【ChatGPT】** 参照束の成果物は **ZIPファイル（`<DocSet>_<Mode>[_<CaseName>].zip`）を唯一の正**とする。ZIP内に INDEX.md / TREE.md / MANIFEST.csv / PATH_INDEX.md / SKIPPED.txt / partファイル群が含まれる。
...
参照束には最低限、INDEX（目的・唯一の正・参照束ID・含有物の概要）、TREE（フォルダ構造）、MANIFEST（ファイル一覧）を含める（スクリプトが自動生成）。ChatGPT版は PATH_INDEX（指定パスの解決結果）と SKIPPED（不足・除外）も確認対象とする。
...
- `00_ai_consult_operation_rules.md` の Version / Updated
- 対象スクリプトのヘッダー（Version / Updated等）
- 対象仕様書の Version / Updated（仕様変更を伴う場合は必須）
...
1. AIが修正案を**コピー＆ペースト可能なMarkdown**または適用用 `.py` スクリプトとして提示する
2. ユーザーがローカルで修正を反映する
3. AIが fresh include / diff bundle を基準に整合性を確認 → 問題なければ「仕様書確定」
...
- コマンド例は原則として1行で提示する
- コマンド例は出力前に末尾まで完全に記述されていることを確認する。引数途中で途切れたコマンドを提示しない
...
- `.py` には以下を含める：
  - 対象ファイルの存在確認
  - 重複防止（2回目実行時に二重適用しない）
  - **パッチスクリプト生成前に、対象ファイルを `rb` モードで読み込み、改行コードをバイト列で可視化して確認してから検索文字列を確定すること。bundleのコードブロック内の表示を改行コードの根拠にしてはならない**
...
## 11. 参照束ID・PATH_INDEX運用ルール

- 参照確定は、添付された参照束の INDEX / TREE / MANIFEST / PATH_INDEX / SKIPPED を基準に行う
- DocSetは bundle 生成時の参照束IDとして扱う。通常ドキュメントの手動更新番号として扱わない
- 通常docsやTODOに「DocSet管理」を要求しない
- **【ChatGPT】** PATH_INDEX.md は指定パス・include set・解決結果・最終含有パスの確認に使う
- **【ChatGPT】** SKIPPED.txt が `(none)` でない場合、missing / excluded が相談に影響しないか確認してから作業する
- AIは PATH_INDEX.md / MANIFEST.csv に含まれないパスを、推測で「存在する前提」にしない
...
3. 作業コードの修正と本手順2のTODO更新をまとめてstage → commit → pushする
   ※ 作業コミットがすでに完了している場合は、TODO更新単独でcommit・pushする
4. 引き継ぎ文を作成する
...
【ChatGPT】
```bash
cd <your-repo>; python ai-consult-tools/chatgpt/consult_bundle_chatgpt.py --mode include --repo-root <your-repo> --case-name "<相談名>" --include-set "common_rules" --include-paths "<TODOドキュメントのパス>" "<現在作業に必要な実ファイル>"
```
...
- **【ChatGPT】** PATH_INDEX.md / SKIPPED.txt を確認し、不足・除外があれば先に報告する
...
仕様の整理・横断観点の抜け漏れチェック・文書化・差分レビュー（diff）を担う。仕様・判断・合意の中核をAIが行い、推測を排除する。最終的な仕様根拠は常に参照束で担保する。
...
- 仕様と実装のズレをゼロにする
- 推測による誤実装を防ぐ
- 参照束ID・INDEX・TREE・MANIFEST・PATH_INDEXにより根拠混乱を防ぐ
- 長期開発でも整合性を保つ
...
- 必要に応じてVersion / Updatedを更新しながら改訂する
