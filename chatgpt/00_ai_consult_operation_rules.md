...
> Updated: 2026-06-04 19:20
> DocSet: 202606041920
> Version: 1.7.8
> Note: v1.7.8 では SQL 生成物を diagnostics に限定せず、seed / migration / repair / verification / ad hoc investigation を含めて原則 `.sql` ファイル化するルールを明確化。
...
- 明示パス指定（例：`pavilion-ellese/public_html/index.php`）が最も安全。
...
```powershell
cd C:\xampp\htdocs; pwsh -NoProfile -ExecutionPolicy Bypass -File ai-consult-tools\chatgpt\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\xampp\htdocs" -CaseName "<相談名>" -IncludePaths "ai-consult-tools/chatgpt/00_ai_consult_operation_rules.md,docs/todo/03_publication_readiness_todo_plan.md"
```
