...
- 相談内容が「相談基盤そのものの改修」である
  - 対象例：`tools/chatgpt/make_consult_bundle.ps1`、またはその仕様書（`01_make_consult_bundle_spec.md` 等）
...
```powershell
cd C:\xampp\htdocs; pwsh -NoProfile -ExecutionPolicy Bypass -File tools\chatgpt\make_consult_bundle.ps1 -Mode diff -RepoRoot "C:\xampp\htdocs" -CaseName "<相談名>"
```
...
- 一時 patch は適用後に削除してよいが、`tools/chatgpt/consult_case/` はユーザーが明示しない限り削除しない。
...
- `tools\chatgpt\consult_case\` などのサブフォルダへのパスを `-File` に指定しない
...
```powershell
cd C:\xampp\htdocs; pwsh -NoProfile -ExecutionPolicy Bypass -File tools\chatgpt\make_consult_bundle.ps1 -Mode include -RepoRoot "C:\xampp\htdocs" -CaseName "<相談名>" -IncludePaths "tools/chatgpt/00_ai_consult_operation_rules.md,docs/todo/03_publication_readiness_todo_plan.md"
```
