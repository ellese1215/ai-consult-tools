...
function Resolve-ConsultConfigPath([string]$repoFull, [string]$configPath) {
    ...
    Write-Host "     例: Copy-Item ai-consult-tools\chatgpt\consult.config.example.json ai-consult-tools\chatgpt\consult.config.json"
    Write-Host "  2. -ConfigPath オプションで設定ファイルのパスを明示する"
    Write-Host "     例: -ConfigPath \"ai-consult-tools\chatgpt\consult.config.json\""
    Write-Host ""
