...
旧ChatGPT版・Claude版スクリプトとその文書は、V4-6まで互換対象として保持する旧版資料であり、本書の現行CLI仕様には含めない。
...
以下は旧ChatGPT版・Claude版スクリプトの仕様であり、現行共通CLIには含めない。
...
以下の旧トップレベル設定キーも現行共通設定には含めない。現行設定の`outputs.chatgpt.outRoot`および`outputs.claude.outRoot`とは別物である。
...
旧スクリプトはV4-6で共通CLIを呼ぶ互換ラッパーへ置換する。V4-5では実装を変更しない。
