...
- 共通の旧引数変換モジュールを追加した
- ChatGPT版・Claude版旧スクリプトを薄い互換ラッパーへ置換した
- 互換入口の引数変換、固定target、終了コード、拒否条件を13件の自動試験で確認した
- READMEと現行技術仕様へ互換入口の現行契約を反映した
- 全209件の試験が成功し、Windowsのsymlink作成権限による既存3件だけがskipとなった

#### V4-6C 旧構成整理：公開構成整理済み・構造同期待ち

- 旧モデル別仕様書、README、ガイド、テンプレート、設定例を削除した
- Git管理外の`local/chatgpt/`と`local/claude/`は、共通local構成への移行確認後に削除した
- `tree.txt`と`duplicate_filenames_report.md`を削除した
- `.gitignore`から旧モデル別設定・local文書向けの重複規則を削除した
- 互換ラッパー以外の旧モデル別資料が通常配布対象へ残らない構成にした
- 構造正本とローカル構造インデックスの同期は、monorepo内の別案件による未確定構造を混在させないため、V4-6Cの最終確認へ残す
